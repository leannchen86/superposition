...
class VectorAnimation(ThreeDScene):
    def construct(self):
        # Set camera to perspective view with explicit distance for zoom control
        start_distance = 8.0
        self.set_camera_orientation(phi=60 * DEGREES, theta=45 * DEGREES, distance=start_distance)
        ...
        # Cinematic camera move: tilt to a top-down 45° view and slowly zoom in
        self.move_camera(phi=45 * DEGREES, theta=45 * DEGREES, run_time=3, rate_func=smooth)
        # Replace camera.frame scale with distance change for compatibility
        self.move_camera(distance=start_distance * 0.8, run_time=5, rate_func=smooth)
        self.wait(1)